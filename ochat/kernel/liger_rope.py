...
class LigerRopeFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
        """
        q size: (1, seq_len, n_q_head, head_dim)
        k size: (1, seq_len, n_kv_head, head_dim)
        cos size: (1, seq_len, head_dim)
    ...
    def backward(ctx, dq, dk):
        """
        dq size: (1, seq_len, n_q_head, head_dim)
        dk size: (1, seq_len, n_kv_head, head_dim)
        cos size: (1, seq_len, head_dim)
        sin size: (1, seq_len, head_dim)
        """
...
def liger_rope_embedding(Q, K, cos, sin):
    Q, K = LigerRopeFunction.apply(Q.unsqueeze(0), K.unsqueeze(0), cos.unsqueeze(0), sin.unsqueeze(0))
