...
def parse_args() -> Tuple[argparse.Namespace, argparse.Namespace, argparse.Namespace, argparse.Namespace]:
...
def train(args: TrainingArguments):
    ...
    for epoch in range(args.epochs):
        ...
        for (batch_tensor, batch_info), all_numseq, cur_numseq in train_loader:
            ...
            # Update
            loss, acc = model_engine(**batch_tensor, **batch_info, num_seq=all_numseq, use_fast_rope=args.use_fast_rope).loss
            ...
            if eval_loader is not None and (args.eval_strategy == 'step' and args.eval_every and (step % args.eval_every == 0)):
                ...
                with torch.inference_mode():
                    for (batch_tensor, batch_info), all_numseq, cur_numseq in eval_loader:
                        ...
                        # Eval
                        eval_loss, eval_acc = model_engine(**batch_tensor, **batch_info, num_seq=all_numseq, use_fast_norm=args.use_fast_norm, use_fast_rope=args.use_fast_rope).loss
