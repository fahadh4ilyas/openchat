...
""" PyTorch Unpadded & Fused Qwen3_5 Moe model. Compatible with HF. """
...
from transformers.models.qwen3_5_moe.modeling_qwen3_5_moe import Qwen3_5MoeVisionModel, Qwen3_5MoeGatedDeltaNet, Qwen3_5MoeRMSNorm, Qwen3_5MoeVisionRotaryEmbedding
...
from ochat.training_utils._ce_utils import weighted_cross_entropy, compute_unpadded_lm_loss
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->Qwen3_5Moe
class UnpaddedQwen3_5MoeRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps):
        """
        UnpaddedQwen3_5MoeRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class UnpaddedQwen3_5MoeRotaryEmbedding(torch.nn.Module):
    def __init__(
        self,
        head_dim,
        rope_theta=10000,
...
class Qwen3_5MoeTopKRouter(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        router_logits = torch.nn.functional.linear(hidden_states, self.weight)  # (seq_len, num_experts)
...
class UnpaddedQwen3_5MoeSparseMoeBlock(nn.Module):
    def __init__(self, config: Qwen3_5MoeTextConfig):
        super().__init__()
        self.gate = Qwen3_5MoeTopKRouter(config)
        self.experts = Qwen3_5MoeExperts(config)
        self.shared_expert = UnpaddedQwen3_5MoeMLP(config, intermediate_size=config.shared_expert_intermediate_size)
...
class UnpaddedQwen3_5MoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3_5MoeTextConfig, layer_idx: Optional[int] = None):
        super().__init__()
        ...
        self.q_norm = UnpaddedQwen3_5MoeRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = UnpaddedQwen3_5MoeRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
...
class UnpaddedQwen3_5MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen3_5MoeTextConfig, layer_idx: Optional[int] = None):
        super().__init__()
        ...
        if self.layer_type == "linear_attention":
            ...
            self.self_attn = UnpaddedQwen3_5MoeAttention(config=config, layer_idx=layer_idx)
        self.mlp = UnpaddedQwen3_5MoeSparseMoeBlock(config=config)
        self.input_layernorm = UnpaddedQwen3_5MoeRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = UnpaddedQwen3_5MoeRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class UnpaddedQwen3_5MoePreTrainedModel(PreTrainedModel):
    config: Qwen3_5MoeConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _supports_flash_attn = True
    _no_split_modules = ["UnpaddedQwen3_5MoeDecoderLayer", "Qwen3_5MoeVisionBlock"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (Qwen3_5MoeRMSNorm, UnpaddedQwen3_5MoeRMSNorm)):
...
class UnpaddedQwen3_5MoeTextModel(UnpaddedQwen3_5MoePreTrainedModel):
    ...
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`UnpaddedQwen3_5MoeDecoderLayer`]
    ...
    def __init__(self, config: Qwen3_5MoeTextConfig):
        ...
        self.rotary_emb = UnpaddedQwen3_5MoeRotaryEmbedding(
        ...
        self.layers = nn.ModuleList(
            [
                UnpaddedQwen3_5MoeDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = UnpaddedQwen3_5MoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class UnpaddedQwen3_5MoeModel(UnpaddedQwen3_5MoePreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`UnpaddedQwen3_5MoeDecoderLayer`]
    ...
    def __init__(self, config: Qwen3_5MoeConfig):
        ...
        self.language_model = UnpaddedQwen3_5MoeTextModel._from_config(config.text_config)
    ...
    def forward(
        ...
        nz_hidden_states, router_logits = self.language_model(
            nz_hidden_states=nz_hidden_states,
...
class Qwen3_5MoeForConditionalGeneration(UnpaddedQwen3_5MoePreTrainedModel):
    _checkpoint_conversion_mapping = {}
    _tied_weights_keys = {"lm_head.weight": "model.language_model.embed_tokens.weight"}
    def __init__(self, config: Qwen3_5MoeConfig):
        super().__init__(config)
        self.model = UnpaddedQwen3_5MoeModel(config)
    ...
    def forward(
        ...
        if nz_shifted_label_ids is not None:
            ...
            (ce_loss, acc), per_seq_logps = compute_unpadded_lm_loss(
                ...
                lm_head=self.lm_head, num_tokens=num_seq,
                return_per_seq_logps=return_per_seq_logps,
            )

            loss = ((ce_loss + aux_loss, aux_loss), acc)
            return CausalLMOutputWithPast(loss=loss, logits=per_seq_logps)
