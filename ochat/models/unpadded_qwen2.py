...
class UnpaddedQwen2Attention(nn.Module):
    ...
    def forward(
        ...
        use_sliding_window = self.use_sliding_window and self.layer_idx < self.max_window_layers
        ...
        if cu_seqlens[-1] == max_seqlen:
            attn_output = flash_attn_func(
                ...
                window_size=(self.sliding_window, self.sliding_window) if use_sliding_window is not None else (-1, -1))
        else:
...
class UnpaddedQwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        self.input_layernorm = UnpaddedQwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = UnpaddedQwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class PaddedQwen2ForCausalLM(Qwen2ForCausalLM):
