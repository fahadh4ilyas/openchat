...
# @torch.jit.script
def lm_head_with_loss(embed_weights: torch.Tensor, hidden_states: torch.Tensor, nz_shifted_label_ids: torch.Tensor, nz_shifted_loss_weights: torch.Tensor, total_seqs: int):
    logits = nn.functional.linear(hidden_states, embed_weights)

    token_accuracy = (nz_shifted_loss_weights * (torch.argmax(logits.detach(), dim=-1) == nz_shifted_label_ids)).sum() / total_seqs
    loss = (nz_shifted_loss_weights * cross_entropy_loss(logits, nz_shifted_label_ids, inplace_backward = True)[0]).sum() / total_seqs
