"""Ring-attention LoRA DPO training entry point.

Uses ring-attention to split long sequences across GPUs for both chosen and
rejected sides of each DPO pair. Only LoRA/QLoRA is supported.
"""
...
class TrainingArguments(BaseTrainingArguments, LoraTrainingArgsMixin):
    """Ring-attention DPO training arguments (LoRA only, base_lr=1e-2)."""
    base_lr: float = 1e-2
    dpo_beta: float = 0.1
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for (chosen_t, rejected_t, chosen_ref, rejected_ref, batch_info), total_seqs in train_loader:
            step += 1
            ...
            if ref_logps_precomputed:
                ...
                with torch.no_grad():
                    chosen_ref = _forward_and_logp_ring(model_engine, chosen_t, batch_info, args, total_seqs)
                    rejected_ref = _forward_and_logp_ring(model_engine, rejected_t, batch_info, args, total_seqs)
                model_engine.module.enable_adapter_layers()

            # Forward: policy log-probs
            chosen_logp = _forward_and_logp_ring(model_engine, chosen_t, batch_info, args, total_seqs)
            rejected_logp = _forward_and_logp_ring(model_engine, rejected_t, batch_info, args, total_seqs)
            ...
            if eval_loader is not None and args.eval_strategy == "step" and args.eval_every and (step % args.eval_every == 0):
                ...
                with torch.inference_mode():
                    for (chosen_t, rejected_t, chosen_ref, rejected_ref, batch_info), total_seqs in eval_loader:
                        chosen_t = {k: (v.to(args.device) if isinstance(v, torch.Tensor) else v) for k, v in chosen_t.items()}
                        rejected_t = {k: (v.to(args.device) if isinstance(v, torch.Tensor) else v) for k, v in rejected_t.items()}
