    ...
    for epoch in range(args.epochs):
        ...
        for (chosen_t, rejected_t, chosen_ref, rejected_ref, batch_info), total_seqs in train_loader:
            ...
            if RANK == 0:
                ...
                if use_cpo:
                    metrics["train/dpo_loss"] = dpo_component.item()
                    metrics["train/sft_loss"] = sft_component.item()
            ...
            if eval_loader is not None and args.eval_strategy == "step" and args.eval_every and (step % args.eval_every == 0):
                model_engine.eval()
                eval_loss, eval_epoch = _eval_loop(model_engine, eval_loader, args, eval_epoch)
                if RANK == 0:
