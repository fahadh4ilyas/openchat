"""Ring-attention ORPO training entry point.

Uses ring-attention to split long sequences across GPUs for both chosen and
rejected sides of each ORPO pair.  Supports both full fine-tuning and
LoRA/QLoRA.
"""
...
class TrainingArguments(BaseTrainingArguments, LoraTrainingArgsMixin):
    """Ring-attention ORPO training arguments (base_lr=3e-4 full FT, 1e-2 LoRA)."""
    orpo_beta: float = 0.1
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for (chosen_t, rejected_t, batch_info), total_seqs in train_loader:
            step += 1
            ...
            if eval_loader is not None and args.eval_strategy == "step" and args.eval_every and (
                ...
                eval_sft, eval_orpo, eval_loss, eval_epoch = _eval_loop(
                    model_engine, eval_loader, args, eval_epoch
                )
                if RANK == 0:
