...
def _save_checkpoint(model_engine, args, save_path, epoch, step, save_tokenizer_too=False):
    ...
    with torch.inference_mode():
        for (batch_tensor, batch_info), all_numseq, cur_numseq in eval_loader:
            ...
            eval_loss, eval_acc = model_engine(
                **batch_tensor, **batch_info, num_seq=all_numseq,
...
@mlflow_stopper_wrapper()
def train(args):
    # Ring attention → delegate to unified train_ring
    ...
    for epoch in range(args.epochs):
        ...
        for (batch_tensor, batch_info), all_numseq, cur_numseq in train_loader:
            ...
            if args.checkpoint_every > 0 and (step % args.checkpoint_every == 0):
                dist.barrier()
                _save_checkpoint(model_engine, args, os.path.join(args.save_path, f"checkpoint_{step}"),
                                 epoch + 1, step)
            ...
            if args.save_strategy == "step" and args.save_every and (step % args.save_every == 0):
                dist.barrier()
                _save_checkpoint(model_engine, args, os.path.join(args.save_path, f"st_{step}"),
