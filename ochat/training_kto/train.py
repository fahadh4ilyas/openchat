...
Supports full fine-tuning, LoRA, and QLoRA in one script.
...
def _run_eval(model_engine, eval_loader, args, eval_epoch, step=None):
    ...
    with torch.inference_mode():
        for (batch_tensor, batch_info, labels, ref_logps), all_numseq, cur_numseq in eval_loader:
...
def train(args):
    if getattr(args, "use_ring", False):
        from ochat.training_kto.train_ring import train as train_ring
    ...
    for epoch in range(args.epochs):
        ...
        for (batch_tensor, batch_info, labels, ref_logps), all_numseq, cur_numseq in train_loader:
            step += 1
